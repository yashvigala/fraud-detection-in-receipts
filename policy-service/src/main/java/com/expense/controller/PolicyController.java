...
public class PolicyController {
    ...
    public ResponseEntity<Map<String, Object>> evaluate(@RequestBody ExpenseClaim claim) {
        ...
        // 2. Compute policy_score AND final status here instead of relying on
        //    R090/R091/R092 in the DRL — those rules read `violations.size()`
        //    and `hardReject`, but the fact-mutating rules don't call
        //    update($c), so the salience-0 pattern matches see stale state.
        //    Doing it in Java is more deterministic and easier to debug.
        int policyScore = 100;
        int hardCount = 0, softCount = 0;
        for (Violation v : claim.getViolations()) {
            policyScore -= v.getDeduction();
            if ("HARD".equalsIgnoreCase(v.getSeverity())) hardCount++;
            else softCount++;
        }
        ...
        if (hardCount > 0) {
            ...
            claim.setPolicyDecision("HARD_FAIL");
        } else if (softCount > 0) {
            claim.setStatus("FLAGGED");
            claim.setPolicyDecision("SOFT_FAIL");
        } else {
            claim.setStatus("APPROVED");
            claim.setPolicyDecision("PASS");
        }
